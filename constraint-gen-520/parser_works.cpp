...
//AN       DEBUG ("\nmapped arguments:");
//AN           DEBUG ("\nlhs var %d, type %d", lhs.var, lhs.type);
//AN            while (VEC_length (ce_s, rhsc) != 0) {
//AN                rhsp = VEC_last (ce_s, rhsc);
//AN        DEBUG ("\nrhs var %d, type %d", rhsp->var, rhsp->type);
...
//AN
void check (vec<fieldoff_s> *fieldstack )
{
...
//AN
bool parser::
push_fields_onto_fieldstack (tree type, vec<fieldoff_s> *fieldstack, HOST_WIDE_INT offset)
{
	DEBUG ("\npush_fields_onto_fieldstack");
	DEBUG ("\nVEC_length (fieldoff_s, *fieldstack)=%d", fieldstack->length());
	unsigned int i = 0;
	fieldoff_s *fo = NULL;
		...
		if (TREE_CODE (field) == FIELD_DECL)
		{
			...
			if (push)
			{
				...
				DEBUG ("\nVEC_empty (fieldoff_s, *fieldstack)=%d, VEC_length (fieldoff_s, *fieldstack)=%d",
				       fieldstack->length() == 0, fieldstack->length());
				...
				if (!fieldstack->length() == 0)
				{
					...
					check (fieldstack);
					pair = &(fieldstack->last()); //VEC_last (fieldoff_s, *fieldstack);
				}
				        ...
				        && offset + foff != 0)
				{
					...
					DEBUG ("\nbefore VEC_length (fieldoff_s, *fieldstack)=%d", fieldstack->length()); //VEC_length (fieldoff_s, *fieldstack));
					fieldoff_s null;
					pair = fieldstack->safe_push(null); //VEC_safe_push (fieldoff_s, heap, *fieldstack, NULL);
					pair->offset = 0;
					...
					DEBUG ("\nafter VEC_length (fieldoff_s, *fieldstack)=%d", fieldstack->length()); //VEC_length (fieldoff_s, *fieldstack));
				...
				else
				{
					...
					DEBUG ("\nbefore VEC_length (fieldoff_s, *fieldstack)=%d", fieldstack->length());
					check (fieldstack);
					fieldoff_s null;
					pair = fieldstack->safe_push(null); //VEC_safe_push (fieldoff_s, heap, *fieldstack, NULL); // PROBLEM: fieldstack not working
					check (fieldstack);
...
cs_get_constraint_for_rhs (tree t, vec<ce_s> *results, basic_block bb, struct cgraph_node * cnode)
{
...
cs_create_variable_info_for_1 (tree decl, const char *name, struct cgraph_node * cnode)
{
	...
	auto_vec<fieldoff_s> fieldstack; // VEC (fieldoff_s,heap) *fieldstack = NULL;
	fieldoff_s *fo;
	unsigned int i;
	varpool_node *vnode;
	...
	if (var_can_have_subvars (decl)
		...
		if (notokay)
			fieldstack.release(); // VEC_free (fieldoff_s, heap, fieldstack);

	}
	DEBUG ("\n3******************************************************************\n");
	if (fieldstack.length() <= 1) //if (VEC_length (fieldoff_s, fieldstack) <= 1)
		DEBUG ("\n<=1");
	if (fieldstack.length() > MAX_FIELDS_FOR_FIELD_SENSITIVE) //if (VEC_length (fieldoff_s, fieldstack) > MAX_FIELDS_FOR_FIELD_SENSITIVE)
		DEBUG ("\n> MAX_FI... %d", MAX_FIELDS_FOR_FIELD_SENSITIVE);
	else
		DEBUG ("\n< MAX_FI... %d", MAX_FIELDS_FOR_FIELD_SENSITIVE);
...
cs_create_variable_info_for (tree decl, const char *name, basic_block bb, struct cgraph_node * cnode)
{
	...
	for (; vi; vi = vi->next) {
		...
		if (DECL_INITIAL (decl)) {
			...
			//print_node (dump_file, "", DECL_INITIAL (decl), 0);
#endif
			vec<ce_s> rhsc; // VEC (ce_s, heap) *rhsc = NULL;
			...
			cs_process_constraint (new_constraint (lhs, *rhsp), bb);
			rhsc.release(); // VEC_free (ce_s, heap, rhsc);     // Vini: Why commented out????
		}
	}
...
//AN   asprintf (&tempname, "%s.%d", name, VEC_length (csvarinfo_t, csvarmap));
...
void parser::
cs_do_deref (vec<ce_s> *constraints, basic_block bb, struct cgraph_node * cnode)
{
	DEBUG ("\ncs_do_deref()");
...
cs_get_constraint_for_ptr_offset (tree ptr, tree offset,
                                  vec <ce_s> *results, basic_block bb, struct cgraph_node * cnode)
{
		...
		/* If we do not know the offset add all subfields. */
			...
			do {
				...
				if (c2.var != c.var)
				{
					...
					//AH VEC_safe_push (ce_s, heap, *results, &c2);
...
//AN    as the lhs.  */
void parser::
cs_get_constraint_for_component_ref (tree t, vec<ce_s> *results,
                                     bool address_p, bool lhs_p, basic_block bb, struct cgraph_node * cnode)
{
	...
	if (CONSTANT_CLASS_P (forzero) && integer_zerop (forzero)) {
		...
		results->safe_push (temp); //AH VEC_safe_push (ce_s, heap, *results, &temp);
		return;
	}
	...
	//AH results->length()if (VEC_length (ce_s, *results) == 0)
	if (results->length() == 0)
		return;
	else
		gcc_assert (results->length() == 1); //AH gcc_assert (VEC_length (ce_s, *results) == 1);


	result = &(results->last()); //AH result = VEC_last (ce_s, *results);
#if DEBUG_CONTAINER
	...
	if (result->type == SCALAR
		...
		if ((unsigned HOST_WIDE_INT)bitpos < cs_get_varinfo (result->var)->fullsize
			...
			csvarinfo_t curr;
			results->pop(); //AH VEC_pop (ce_s, *results);
			cexpr.offset = 0;
			for (curr = cs_get_varinfo (cexpr.var); curr; curr = curr->next) {
				if (ranges_overlap_p (curr->offset, curr->size,
					...
					results->safe_push (cexpr); //AH VEC_safe_push (ce_s, heap, *results, &cexpr);
					if (address_p)
						break;
				}
			}
...
//AN    If address_p is true, the result will be taken its address of.  */
void parser::
cs_get_constraint_for_ssa_var (tree t, vec<ce_s> *results, bool address_p, basic_block bb, struct cgraph_node * cnode)
{
	...
	results->quick_push (cexpr); //AH VEC_safe_push (ce_s, heap, *results, &cexpr);

}
//AN
//AN /* Given a tree T, return the constraint expression for it.  */
void parser::
cs_get_constraint_for_1 (tree t, vec<ce_s> *results, bool address_p,
                         bool lhs_p, basic_block bb, struct cgraph_node * cnode)
{
	...
	if (TREE_CODE (t) == STRING_CST) {
		...
		temp.type = SCALAR;
		temp.offset = 0;
		results->safe_push(temp); //VEC_safe_push (ce_s, heap, *results, &temp);
	...
	case tcc_expression:
	{
		...
		case ADDR_EXPR:
			...
			DEBUG ("\noffset seq 1 %d", results->last().offset_sequence);
			return;
		default:;
		}
		break;
	...
	case tcc_reference:
	{
		...
		case MEM_REF:
		{
			...
			DEBUG ("\noffset seq 2 %d", results->last().offset_sequence);
			...
			if (!vi->is_full_var && curr) {
				...
				for (; curr; curr = curr->next) {
					if (curr->offset - vi->offset < size) {
						cs.var = curr->id;
						results->safe_push(cs); //VEC_safe_push (ce_s, heap, *results, &cs);
		...
		case COMPONENT_REF:
			...
			DEBUG ("\noffset seq 3 %d", results->last().offset_sequence);
#if DEBUG_CONTAINER
		...
		case VIEW_CONVERT_EXPR:
			...
			DEBUG ("\noffset seq 4 %d", results->last().offset_sequence);
			return;
	...
	case tcc_exceptional:
	{
		...
		case SSA_NAME:
		{
			...
			DEBUG ("\noffset seq 5 %d", results->last().offset_sequence);
			return;
		}
		case CONSTRUCTOR:
		{
			...
			FOR_EACH_CONSTRUCTOR_VALUE (CONSTRUCTOR_ELTS (t), i, val) {
				...
				tmp.truncate (0);
			} DEBUG ("\noffset seq 6 %d", results->last().offset_sequence);
	...
	case tcc_declaration:
	{
		DEBUG ("\ntcc_declaration");
		cs_get_constraint_for_ssa_var (t, results, address_p, bb, cnode);
		DEBUG ("\noffset seq 7 %d", results->last().offset_sequence);
		return;
	}
	case tcc_constant:
		DEBUG ("\ntcc_constant");
...
cs_process_all_all_constraints (vec<ce_s> lhsc, vec<ce_s> rhsc, basic_block bb)
{
...
//AN    address of it. */
void parser::
cs_get_constraint_for_address_of (tree t, vec<ce_s> *results, basic_block bb, struct cgraph_node * cnode)
{
	struct constraint_expr *c;
	unsigned int i;
...
cs_get_constraint_for (tree t, vec<ce_s> *results, basic_block bb, struct cgraph_node * cnode)
{
	CERR (cs_get_constraint_for 1 % % % % % % % % % % % % );
	VEC_length (ce_s, *results); //AN
...
//    }
	...
	if (lhsp->type == DEREF || rhsp->type == DEREF)
	{
		...
		DEBUG ("\nVEC_length (ce_s, lhsc) = %d, VEC_length (ce_s, rhsc) = %d", lhsc.length(), rhsc.length());
...
insert_alias_in_pool (constraint_t t, csvarinfo_t vi, basic_block bb)
{
	...
	if (!alias_found) {
		//AH loc = VEC_length (constraint_t, aliases);
		//AH VEC_safe_push (constraint_t, heap, aliases, t);
		loc = aliases.length();
		aliases.safe_push (t);
...
cs_process_constraint (constraint_t t, basic_block bb)
{
	...
	gcc_assert (rhs.var < csvarmap.length());
	gcc_assert (lhs.var < csvarmap.length());
...
process_gimple_phi_stmt (gimple stmt, basic_block bb, struct cgraph_node * cnode)
{
	...
	for (i = 0; i < gimple_phi_num_args (stmt); i++)
	{
		...
		for (j = 0; lhsc.iterate(j, &c); j++)
		{
			...
			{
				DEBUG ("\nrhsc VEC loop");
				struct constraint_expr &c2 = rhsc.last(); //VEC_last (ce_s, rhsc);
...
//AN
//AN    DEBUG ("\nVariable data:");
//AN    for (unsigned int index = 0; index < VEC_length (csvarinfo_t, variable_data); index++)
//AN    {
...
//AN
//AN    for (int index = 0; index < VEC_length (csvarinfo_t, variable_data); index++)
//AN    {
...
print_variable_data ()
{
	...
	for (int index = 0; index < variable_data.length(); index++)
	{
...
print_assignment_data ()
{
	...
	for (int index = 0; index < assignment_data.length(); index++)
	{
		constraint_t assignment = assignment_data[index]; //VEC_index (constraint_t, assignment_data, index);
		constraint_expr lhs = assignment->lhs;
		constraint_expr rhs = assignment->rhs;
...
//AN
//AN void parser::
//AN print_original_cfg ()
//AN {
//AN #if DEBUG_CONTAINER
//AN    FUNCTION_NAME ();
//AN #endif
//AN
//AN        struct cgraph_node * cnode = NULL;
//AN    for (cnode = cgraph_nodes; cnode; cnode = cnode->next)
//AN    {
//AN        // Nodes without a body, and clone nodes are not interesting.
//AN        if (!gimple_has_body_p (cnode->decl) || cnode->clone_of)
//AN            continue;
//AN
//AN        push_cfun(DECL_STRUCT_FUNCTION (cnode->decl));
...
//AN #endif
//AN        int n = 1;
//AN        basic_block current_block;
//AN        FOR_EACH_BB (current_block)
//AN        {
//AN                    DEBUG ("\nBlock : %d\n", current_block->index);
//AN            gimple_stmt_iterator gsi;
//AN #if DEBUG_CONTAINER
//AN            for (gsi = gsi_start_bb (current_block); !gsi_end_p (gsi); gsi_next (&gsi))
//AN            {
//AN                DEBUG ("%d: ", n++);
//AN                print_gimple_stmt (dump_file, gsi_stmt (gsi), 0, 0);
//AN            }
